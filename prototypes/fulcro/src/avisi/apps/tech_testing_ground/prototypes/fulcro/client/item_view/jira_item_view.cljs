                      ...
                      (dropdown/ui-dropdown-item-group {}
                                                       (if monday-item-id
                                                         (dropdown/ui-dropdown-item
                                                           {:onClick #(comp/transact! this [(domain/delete-item-link {:platform "jira" :board-id board-id :item {:id item-id}})])}
                                                           "Delete ItemLink")
                                                         (dropdown/ui-dropdown-item
          ...
          {:root ItemLinkView
           ...
           (fn [app]
             ...
             (js/AP.events.on "item-link-created"
                              (fn [data]
                                (comp/transact! app [(domain/add-item-link (edn/read-string data))])))
