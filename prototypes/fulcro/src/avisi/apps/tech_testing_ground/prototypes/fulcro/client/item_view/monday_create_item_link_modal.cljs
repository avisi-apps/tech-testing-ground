...
(defmutation create-item-link [_]
  (action [{:keys [app]}] (df/set-load-marker! app :item-link :creating))
  (remote [env]
    (->
      env
      (m/with-server-side-mutation 'create-item-link)))
  (ok-action [{:keys [app]}] (df/remove-load-marker! app :item-link) (monday.execute "closeAppFeatureModal")))

(defsc ItemLinkModal
  [this
   {:keys [selected-item available-items]
    :as props}]
  {:query
  ...
  (let [jira-item-id (comp/get-state this :jira-item-id)
    ...
    (modal-dialog/ui-modal
      {}
      (modal-dialog/ui-modal-header {} (modal-dialog/ui-modal-title {} "Create ItemLink"))
               ...
               #(comp/transact!
                   ...
                   [(create-item-link
                      {:source
                         {:platform "monday"
                          :board-id board-id
                          :item {:item/id (str monday-item-id)}}
