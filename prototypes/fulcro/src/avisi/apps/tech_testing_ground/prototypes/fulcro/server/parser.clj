...
(defn api-parser [{:keys [request query]}]
  ...
  (try
    (p.eql/process (assoc env :request request) query)
    (catch Exception e (prn (ex-message e)))))


(comment (api-parser {:query [{:hello-world [:message]} {:x [:message]}]})
         (api-parser {:query [{:x [:message]}]})
         (api-parser {:query [({:available-items [:item/id]} {:platform "jira", :board-id 10002})]})
         (api-parser {:query [({:item [:item/id]} {:board {:platform "jira", :board-id 10002} :item {:item/id "ME-126"}})]})
         )
