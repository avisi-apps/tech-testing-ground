...
(defmethod webhook-event->monday-item "update_column_value"
  [{id :pulseId
    {{status :text} :label} :value
    :as req}]
  {:item/id id
   :item/status status})

(defmethod webhook-event->monday-item "delete_pulse" [{id :itemId}] {:item/id id})
...
(defn webhook-handler [req] (def _r req) (try (propagate-action req) {:status 200} (catch Exception _ {:status 500})))
