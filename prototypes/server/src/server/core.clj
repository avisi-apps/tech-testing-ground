...
(def not-found-response
  {:status 404
   ...
   :body "Not Found"})
(defn- wrap-index-as-root [next-handler]
  (fn [request]
    (next-handler
      (if (= "/" (:uri request))
...
(defn start-server [{:keys [port] :as server-config}]
  (jetty/run-jetty (app server-config) {:port port
